...
def results_to_dict_timestamps(results):
    """convert result arrays into dict used by json files"""
    # video ids and allocate the dict
    vidxs = sorted(list(set(results['video-id'])))
    results_dict = {}
...
def results_to_array_timestamps(results, num_pred):
    # video ids and allocate the dict
    vidxs = sorted(list(set(results['video-id'])))
    results_dict = {}
    for vidx in vidxs:
    ...
    # fill in the dict
    for vidx, time, label, score in zip(
        results['video-id'],
        results['time'],
        results['label'],
        results['score']
        ...
        results_dict[vidx]['label'].append(int(label))
        results_dict[vidx]['score'].append(float(score))
        results_dict[vidx]['time'].append(float(time))

    for vidx in vidxs:
        label = np.asarray(results_dict[vidx]['label'])
        score = np.asarray(results_dict[vidx]['score'])
        time_v = np.asarray(results_dict[vidx]['time'])

        # the score should be already sorted, just for safety
        inds = np.argsort(score)[::-1][:num_pred]
        results_dict[vidx]['label'] = label[inds]
        results_dict[vidx]['score'] = score[inds]
        results_dict[vidx]['time'] = time_v[inds]

    return results_dict


def postprocess_results_timestamps(results, cls_score_file, num_pred=200, topk=2):

    # load results and convert to dict
    if isinstance(results, str):
        results = load_results_from_pkl(results)
    # array -> dict
    results = results_to_array_timestamps(results, num_pred)
    ...
    for vid, result in results.items():
        ...
        # model outputs
        pred_score, pred_time, pred_label = \
            result['score'], result['time'], result['label']
        num_segs = min(num_pred, len(pred_score))

        # duplicate all segment and assign the topk labels
        # K x 1 @ 1 N -> K x N -> KN
        # multiply the scores
        # goal of fusion: pred_scores tell me how likely a class is for this video
        # times tell me where timestamps could be
        # by multiplication/combining, a timestamp only stays strong if both the class is likely and the timestamp looks
        # like a peak
        new_pred_score = np.sqrt(topk_cls_score[:, None] @ pred_score[None, :]).flatten()
        new_pred_time = np.tile(pred_time, (topk, 1)).flatten()  # NEW
        new_pred_label = np.tile(topk_cls_idx[:, None], (1, num_segs)).flatten()  # SAME SHAPE

        # add to result
        processed_results['video-id'].extend([vid]*num_segs*topk)
        processed_results['time'].append(new_pred_time)
        processed_results['label'].append(new_pred_label)
        processed_results['score'].append(new_pred_score)

    processed_results['time']  = np.concatenate(
        processed_results['time'],  axis=0)
    processed_results['label'] = np.concatenate(
        processed_results['label'],axis=0)
    processed_results['score'] = np.concatenate(
        processed_results['score'], axis=0)
