...
@torch.jit.script
def categorical_focal_loss_jit(
        logits: torch.Tensor,
        targets: torch.Tensor,
        gamma: float = 2.0,
    ...
    """
    JIT-compiled calculation for speed and stability.
    """
    num_classes = logits.size(1)

    # 1. Create Smoothed One-Hot Targets
    if label_smoothing > 0.0:
        smooth_val = label_smoothing / num_classes
        confidence = 1.0 - label_smoothing + smooth_val
    ...
    # Extract probability of the correct class
    # We use the target indices to gather the specific probabilities
    p_t = probs.gather(1, targets.unsqueeze(1))
    focal_weight = (1.0 - p_t) ** gamma

    # 4. Calculate Loss
    # Cross Entropy: -sum(target * log_prob)
    # Weighted by focal weight
    loss = -(one_hot * log_probs) * focal_weight

    return loss.sum(dim=1).mean()


# --- PART 2: The Module Wrapper (For easy usage) ---
class CategoricalFocalLoss(nn.Module):
    ...
    def forward(self, logits, targets):
        return categorical_focal_loss_jit(
            logits,
            targets,
